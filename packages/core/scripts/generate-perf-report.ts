...
const PERFORMANCE_TARGETS: PerformanceTarget[] = [
  ...
  { name: 'Open 100k cells', metric: 'open_100k_cells', target: 1000, unit: 'ms', comparison: 'lte' },
  { name: 'Open 1M cells', metric: 'open_1m_cells', target: 3000, unit: 'ms', comparison: 'lte' },
  { name: 'Recalc 10k formulas', metric: 'recalc_10k_formulas', target: 500, unit: 'ms', comparison: 'lte' },
  { name: 'Recalc 100k formulas', metric: 'recalc_100k_formulas', target: 2000, unit: 'ms', comparison: 'lte' },
  ...
  { name: 'Filter 100k rows', metric: 'filter_100k_rows', target: 500, unit: 'ms', comparison: 'lte' },
  { name: 'Memory 100k cells', metric: 'memory_100k_cells', target: 100, unit: 'MB', comparison: 'lte' },
  { name: 'Memory 1M cells', metric: 'memory_1m_cells', target: 500, unit: 'MB', comparison: 'lte' },
];
...
function generateReportEntries(
  ...
  for (const result of results) {
    ...
    const deltaPercent = baselineEntry && delta !== undefined
      ? (delta / baselineEntry.value) * 100
      : undefined;

    // Determine status
    let status: 'pass' | 'fail' | 'regression' | 'improvement';
    const targetMet = target.comparison === 'lte'
      ? result.median <= target.target
      : result.median >= target.target;
...
`;
  ...
  for (const entry of entries) {
    ...
    const deltaStr = entry.deltaPercent !== undefined
  ...
  md += `\n---\n*Report generated by MS Excel Clone Performance Validation*\n`;
...
<head>
  ...
  <title>Performance Report - MS Excel Clone</title>
...
<body>
  <div class="container">
    ...
    <footer>
      MS Excel Clone Performance Validation
    </footer>
  </div>
...
async function main(): Promise<void> {
  ...
  if (args.format === 'html' || args.format === 'both') {
    ...
    const htmlPath = args.format === 'html' && args.output
      ? args.output
